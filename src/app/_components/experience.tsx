  ...
  return (
    <section id="experience" className="my-12 px-4 pt-20 -mt-20">
      ...
      <div className="relative">
        ...
        <div className="hidden sm:block absolute left-1/2 top-0 w-px h-full bg-gray-500 transform -translate-x-1/2"></div>

        <ul className="relative">
          {experiences.map((exp, index) => (
            <li
              key={index}
              className={`flex flex-col sm:flex-row items-start mb-12 ${index % 2 === 0 ? 'sm:flex-row' : 'sm:flex-row-reverse'} relative`}
            >
              {/* Time Element */}
              <div className={`w-full sm:w-1/2 flex flex-col self-center ${index % 2 === 0 ? 'sm:pr-4 sm:items-end' : 'sm:pl-4 sm:items-start'} items-start space-y-2`}>
                <time className="timeline-box">{exp.time}</time>
                <h2 className="timeline-title text-xl font-bold">{exp.title}</h2>
              </div>
              
              {/* Horizontal Line */}
              <hr className="w-full border-t-1 bg-gray-500 my-3 sm:hidden" />

              {/* Job Details */}
              <div className={`w-full sm:w-1/2 ${index % 2 === 0 ? 'sm:pl-4' : 'sm:pr-4'} flex items-center`}>
